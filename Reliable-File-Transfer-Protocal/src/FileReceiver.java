...
public class FileReceiver {
	...
	private static List<Pair<Integer, Integer>>emptyarea;
	...
	public static void main(String[] args) throws Exception 
	{
		...
		while(true)
		{
			...
			else
			{
				...
				end = window_and_bits&0x10000;
				title = window_and_bits&0x20000;
				fin = window_and_bits&0x40000;
				syn = window_and_bits&0x80000;
				ack = window_and_bits&0x100000;
				window = window_and_bits&0xffff;

				if(title == 1){
					byte[] name =
					filename = new String(new byte(data));
					fos = new FileOutputStream()
				}
				bb.put(data,20,data.length - 20);
